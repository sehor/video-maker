...
Worker 返回对象 key、大小、SHA-256、duration_ms、width、height、fps、codec，以及 GPU、队列、冷启动、运行时间、计费时间、Worker 和 workflow 版本。Worker 返回成功不等于 Job 成功。
...
2. 校验对象存在、非空、大小上限、声明大小与 SHA-256；现有受控下载/发布流程继续校验字节完整性；
3. 写 Output，原子发布唯一最终结果，再结算秒数。

控制面不启动 FFmpeg/ffprobe，不逐帧解码、不生成预览图。VALID 仅表示元数据与对象完整性通过，不保证视频可播放或生成内容质量。缺失或非法元数据、空对象、尺寸/时长不符、传输校验不符均视为 Attempt 失败；通过解码才能发现的损坏不在同步验收范围。允许把 720-class 原生尺寸 crop／pad 到规范尺寸，禁止放大后冒充 1080p。
