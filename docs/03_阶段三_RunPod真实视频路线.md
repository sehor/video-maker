...
BL-06/07/08 补齐项目素材列表、唯一首帧原子绑定、PATCH 非空校验和任务/下载恢复；
浏览器验收使用启用首帧要求的模拟 I2V 路线，注入生成响应丢失、轮询及下载错误。
这仍是模拟控制面验收；真实 GPU、云存储、Hatchet Cloud 与生产发布须分别验证。

BL-05 增加 `0015_provider_artifacts` 与源/最终对象写入登记，复用 Storage Cleanup 调度循环
执行保留期、租约和重试清理；项目删除清单包含源对象和到期时间，迟到结果可重新打开清理。
...
历史输入不回填，发布须暂停新接单、排空旧活动任务后启用数据库必填保护。
迁移、人工处置清单和回滚限制见 [BL-02A 说明](plans/BL-02A_输入快照契约与迁移.md)。
...
Worker 返回对象 key、duration_ms、width、height、fps、codec，以及 GPU、队列、冷启动、运行时间、计费时间、Worker 和 workflow 版本。Worker 返回成功不等于 Job 成功。

响应契约现为 v2：成功响应必须包含上述媒体元数据，旧响应缺字段会明确拒绝；第三方 Worker/适配层需同步提供真实生成结果元数据，不能从请求参数臆造。真实路线当前仍受发布门禁保护，升级契约不表示已部署或实测远端。

Control Plane 必须：

1. 先检查 Provider 元数据：video/mp4、H.264、1280×720 或 720×1280、与任务一致的画幅和时长（容差 250ms）、有限且大于 0、不超过 240 的帧率；
2. 检查对象存在、非空和大小上限；受控下载/发布不计算或比对媒体哈希，也不比对声明大小；
3. 写 Output，原子发布唯一最终结果，再结算秒数。

控制面不启动 FFmpeg/ffprobe，不逐帧解码、不生成预览图。VALID 仅表示元数据与对象基础检查通过，不保证视频可播放或生成内容质量。缺失或非法元数据、空对象、尺寸/时长不符、对象超限均视为 Attempt 失败；通过解码才能发现的损坏不在同步验收范围。允许把 720-class 原生尺寸 crop／pad 到规范尺寸，禁止放大后冒充 1080p。
...
- [ ] Worker 成功但必需元数据缺失/非法、对象缺失、为空或超限时不会结算；仅解码可发现的损坏不在同步验收范围；
- [ ] 中断后任务可恢复或正确返还；
...
镜像 digest、模型 SHA-256、5 秒横竖屏真实输出、元数据与对象基础检查、冷启动、运行时间、成本、
