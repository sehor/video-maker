...
BL-02A（2026-09-07）新增兼容迁移 `0013_job_input_snapshot` 和不可变输入快照契约。
本步骤不接入 Job 创建或 Provider 执行，强制快照的切换也未在开发库启用；BL-02B 负责
保存并消费快照。历史输入不回填，发布须暂停新接单、排空旧活动任务后启用数据库必填保护。
