...
无凭据时按原生矩阵明确跳过。本阶段结束时，原生剩余必做项为 WINDEV-06 的整体矩阵和
维护文档切换，当时计划保持 `IN_PROGRESS`。

2026-08-31 后续结果：上述 WINDEV-06 已完成，计划状态更新为 `ACCEPTED`，见
[原生总验收报告](WINDEV-06_Windows原生总验收.md)。容器和远程 CI 未执行的事实不变，
不将其列为 Windows 原生改造的剩余必做项。
