...
<template>
  <div>
    <div class="page-head"><div><h1>任务</h1><p class="muted">查看所有生成任务及结果。</p></div></div>
    <div v-if="jobs.length" class="list">
      <NuxtLink v-for="job in jobs" :key="job.id" :to="`/jobs/${job.id}`" class="panel row card-link">
        <div><strong>任务 {{ job.id.slice(0, 8) }}</strong><p class="muted">{{ new Date(job.created_at).toLocaleString() }}</p></div>
